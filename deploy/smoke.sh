...
if [[ "${RUN_ORCHESTRATION_SMOKE:-0}" == "1" ]]; then
  ...
  complete_body='{"result_md":"# Smoke Result\n\nThe worker completed the smoke task. A result exists with passing evidence.","evidence":{"files_changed":[],"smoke_test":"pass","commands":["echo done"]},"status":"ready_for_review"}'
